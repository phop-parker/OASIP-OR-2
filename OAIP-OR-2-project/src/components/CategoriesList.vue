...
<template>
    <div v-if="statusDetail" class=" absolute inset-0 flex justify-center items-center z-20">
        <CategoryDetails :category="curCategory" @closePopUp="toggleStatus()" />
    </div>
    <div class="py-16 overflow-hidden">
        <div class="container m-auto px-6 space-y-8 text-gray-500 md:px-12">
            <div>
                <h2 class="mt-4 text-center text-2xl text-blood-bird font-bold md:text-4xl">Categories
                </h2>
            </div>
            <div
                ...
                <div v-for="(category, index) in categories" :key="index">
                    <div class="relative group bg-white transition hover:z-[1] hover:shadow-xl  ">
                        <div class="relative p-8 space-y-8">
                            <img src="../assets/logo.png" class="w-10" width="512" height="512"
                                alt="burger illustration">
                            <div class="space-y-2">
                                <h5
                                    class="text-xl truncate ... text-gray-800 font-medium transition group-hover:text-yellow-600">
                                    {{ category.eventCategoryName }}</h5>
                                <p class="text-sm text-gray-600">Duration : {{ category.eventDuration }} minutes</p>
                            </div>
                            <a @click="showDetailsToggle(category)"
