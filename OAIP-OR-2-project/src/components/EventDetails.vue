...
<div class="absolute bg-black opacity-20 inset-0 z-0"></div>
    <div class="box-content w-fit h-fit p-8 pb-8 rounded-3xl bg-pastel-orange drop-shadow-3xl font-Kanit z-10">
        <div class="grid grid-rows-8 gap-4  bg-white bg-opacity-20 rounded-2xl p-7 pb-10 w-fit h-fit ">
            <div class="place-self-end ">
                <CloseIcon class="hover:drop-shadow-5xl " @click="$emit('closePopUp'), editMode = false" />
            </div>
            <div class=" text-center text-black bg-white bg-opacity-20">
                Details
            </div>
            ...
            <div v-else-if='event.eventNotes == "" || event.eventNotes == null' class=" pl-3 drop-shadow-xl bg-white bg-opacity-10">
                Note : -
            </div>
            <div v-else class="pl-3 pb-1 drop-shadow-xl bg-white bg-opacity-20 rounded-2xl ">
